...
#define CR4_OSFXSR              0x00000200      // OS FXSAVE/RSTOR support
#define CR4_VMXE                0x00004000      // VMX Enable
#define CR4_FSGSBASE            0x00010000      // [RD/WR]FS/GSBASE support

// eflags bits (useful for rdflags() and wrflags())
...
static __always_inline void insb(int port, void* addr, int cnt) {
...
static __always_inline void insw(int port, void* addr, int cnt) {
...
static __always_inline void insl(int port, void* addr, int cnt) {
...
static __always_inline void outb(int port, uint8_t data) {
    asm volatile("outb %0,%w1" : : "a" (data), "d" (port));
}

static __always_inline void outsb(int port, const void* addr, int cnt) {
...
static __always_inline void outw(int port, uint16_t data) {
    asm volatile("outw %0,%w1" : : "a" (data), "d" (port));
}

static __always_inline void outsw(int port, const void* addr, int cnt) {
...
static __always_inline void outsl(int port, const void* addr, int cnt) {
...
static __always_inline void outl(int port, uint32_t data) {
...
static __always_inline void lidt(void* p) {
    asm volatile("lidt (%0)" : : "r" (p));
}

static __always_inline void lldt(uint16_t sel) {
    asm volatile("lldt %0" : : "r" (sel));
}

static __always_inline void ltr(uint16_t sel) {
    asm volatile("ltr %0" : : "r" (sel));
}

static __always_inline void lcr0(uint32_t val) {
    uint64_t xval = val;
    asm volatile("movq %0,%%cr0" : : "r" (xval));
}

static __always_inline uint32_t rcr0() {
    uint64_t val;
    asm volatile("movq %%cr0,%0" : "=r" (val));
    return val;
}

static __always_inline uintptr_t rcr2() {
    uintptr_t val;
    asm volatile("movq %%cr2,%0" : "=r" (val));
    return val;
}

static __always_inline void lcr3(uintptr_t val) {
    asm volatile("" : : : "memory");
...
static __always_inline void lcr4(uint64_t val) {
    asm volatile("movq %0,%%cr4" : : "r" (val));
}

static __always_inline uint64_t rcr4() {
    uint64_t cr4;
...
static __always_inline bool is_cli() {
    return (rdflags() & EFLAGS_IF) == 0;
}

static __always_inline void tlbflush() {
    uint32_t cr3;
    asm volatile("movl %%cr3,%0" : "=r" (cr3));
    asm volatile("movl %0,%%cr3" : : "r" (cr3));
}

static __always_inline uintptr_t read_rbp() {
    uintptr_t rbp;
    asm volatile("movq %%rbp,%0" : "=r" (rbp));
    return rbp;
}

static __always_inline uintptr_t read_rsp() {
    uintptr_t rsp;
    asm volatile("movq %%rsp,%0" : "=r" (rsp));
    return rsp;
}

static __always_inline x86_64_cpuid_t cpuid(uint32_t leaf) {
    x86_64_cpuid_t ret;
    asm volatile("cpuid"
                 : "=a" (ret.eax), "=b" (ret.ebx),
                   "=c" (ret.ecx), "=d" (ret.edx)
...
static __always_inline x86_64_cpuid_t cpuid(uint32_t leaf, uint32_t subleaf) {
    x86_64_cpuid_t ret;
    asm volatile("cpuid"
                 : "=a" (ret.eax), "=b" (ret.ebx),
                   "=c" (ret.ecx), "=d" (ret.edx)
...
static __always_inline void wrmsr(uint32_t msr, uint64_t x) {
    asm volatile("wrmsr" : : "c" (msr), "a" ((uint32_t) x), "d" (x >> 32));
}

static __always_inline uint64_t rdfsbase() {
    uint64_t x;
    asm volatile("rdfsbase %0" : "=r" (x));
    return x;
}
static __always_inline void wrfsbase(uint64_t x) {
    asm volatile("wrfsbase %0" : : "r" (x));
}

static __always_inline uint64_t rdgsbase() {
    uint64_t x;
    asm volatile("rdgsbase %0" : "=a" (x));
    return x;
}
static __always_inline void wrgsbase(uint64_t x) {
    asm volatile("wrgsbase %0" : : "a" (x));
}
...
static __always_inline void halt() {
